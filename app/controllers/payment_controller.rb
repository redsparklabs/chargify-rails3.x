class PaymentController < ApplicationController
  ...
  def create
   @payment = Payment.new( :card_number =>  params[:payment]['card_number'],  :cvv => params[:payment]['cvv'], :first_name_on_card =>params[:payment]['first_name_on_card'], :last_name_on_card =>params[:payment]['last_name_on_card'],  :billing_zip =>params[:payment]['billing_zip'],:expiration_date =>params[:payment]['expiration_date'], :first_name =>params[:payment]['first_name'], :last_name =>params[:payment]['last_name'], :email_address =>params[:payment]['email_address'], :phone =>params[:payment]['phone'], :organization=>params[:payment]['organization'])
   respond_to do |format|
    if @payment.save
           product_handle = 'REPLACE_PRODUCT_HANDLE' 
